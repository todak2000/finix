...
import { makeMockDeposit } from '@/lib/circle/circle';
import { paymentWarning } from '@/lib/circle/constants';
...
const BankInstructions = ({ data: instructions }: { data: any }) => {
  const handleDeposit = async () => {
    const data = {
      amount: instructions.amount,
      accountNumber: instructions.beneficiaryBank.accountNumber,
      trackingRef: instructions.trackingRef,
    };
    const { data: depositResult } = await makeMockDeposit(data);
    // console.log(depositResult, 'depsoit response---');
  };
  const infoRows = [
    { label: 'Amount to Pay', value: `$${formatAsMoney(instructions.amount)}` },
    { label: 'Beneficiary Name', value: instructions.beneficiary.name },
    {
    ...
    { label: 'Tracking Reference ID', value: instructions.trackingRef },
  ];
  ...
  return (
    <div className="font-roboto flex min-w-[300px] max-w-[320px] flex-col items-center justify-center gap-2 md:max-w-[500px] md:px-12">
      ...
      {/* <button type="button" onClick={handleDeposit}> Click to complete a mock payment</button> */}
    </div>
  );
};
