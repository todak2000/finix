...
import { getWalletBalance } from '@/lib/circle/circle';
...
import { transactions, user } from '@/lib/store';
          ...
          return (
            ...
            >
              ...
              <div className="right flex flex-col gap-3">
                ...
                <p
                  className={cn('text-right text-xs font-normal', {
...
const Dashboard = () => {
  ...
  useEffect(() => {
    async function fetchBalance() {
      ...
      try {
        const { data: walletData } = await getWalletBalance(walletId);
        walletData.balances[0].amount &&
          setBalance(walletData.balances[0].amount);
      } catch (error) {
        /* empty */
      }
    }
        ...
        <>
          ...
          <div className="mt-6 flex w-full flex-col md:flex-row">
            <div className="w-full space-y-6 md:w-1/3">
              <WalletBalance name={walletId} balance={balance} />
              <Converter />
            </div>
